...
3. 环境变量
    ...
    + `export` 自定义变量**转成环境变量** 或者 **显示**所有**环境变量**
...
5. `read` 变量**从键盘**读取值
    ...
    + ```
      ...
      ```
6. `declare/typeset` 声明变量的**类型**
    + `-x`：将后面的变量声明为**环境变量**
    + `-a`：声明为**数组（array）**
    + `-i`：声明为**整型（integer）**
    + `-r`：声明为**只读（readonly）** 不可修改和重设，不小心设置成只读，只能**注销再登录**才能恢复
    + ```
        ...
        echo ${var[1]}
      ```
7. `ulimit` bash限制用户的某些**系统资源**，如可以**打开的文件数量、可以使用的CPU时间、可以使用的内存总量等等**
    + `ulimit -a`
    + `ulimit -f 10240` 如果是**一般用户**，只能**继续减小**文件容量，不能增加文件容量
    + **复原**ulimit设置：注销再登录
8. 变量**内容**的**删除**
    + `#` **从头开始**符合替换模式的**最短的**那一个
    + `##` **从头开始**符合替换模式的**最长的**那一个
    + `%` **从尾向前**符合替换模式的**最短的**那一个
    + `%%` **从尾向前**符合替换模式的**最长的**那一个
    + ```
        path=$PATH
        echo ${path#/*:} //仅删除第一个路径
        echo ${path##/*:} //仅剩下最后一个路径
...
9. 变量**内容**的**替代**
    ...
    + ```
      ...
      ```
11. 变量的**测试**与**内容**的**替换**
    + “判断”某个变量是否存在，**存在**则**使用**既有的设置，**不存在**则**给予**一个常用的设置
    + ps：规则较多，**上网查阅**即可
+ `alias unalias` 命令**别名**
1. ```
    alias lm='ls -l'
