...
# Bash shell的操作**环境**
+ 查找命令的**顺序**，优先级**从高到低**为：
1. 以**相对/绝对路径**执行命令，例如`/bin/ls`或`./ls`
2。 由`alias`找到该命令
3. 由bash**内置（builtin）** 的命令来执行
4. 通过$PATH这个变量的顺序找到的**第一个命令**来执行
+ `/etc/issue` `/etc/motd` bash的登录与**欢迎**信息
+ `/etc/profile` `~/.bash_profile` `~./bash_login` `~/.profile` Bash的**环境配置** 
1. 一进入bash就有一堆**有用变量**是因为系统有一些**配置文件**的存在，bash在**启动时**直接读取这些配置文件（**只在**启动时读取，**之后更改**配置文件**不会生效**的原因，除非用`source`命令让其**实时生效**），配置文件可以分为**全体系统**的配置文件和**用于个人偏好**的配置文件
2. **命令别名**、**自定义变量**在你退出bash后就会**失效**，如果要**保留**设置，则必须讲这些设置**写入**配置文件
3. **login** shell 与 **non-login** shell
    + login shell 需要完整登录流程（账号密码等）；non_login shell不需要（例如在原本bash下启动第二个bash子进程，就不用输入账号密码）
    + login shell 和 non-login shell 读取的配置文件**不一样**
4. **`/etc/profile`**（login shell**才会**读）
    + **每个用户**登录取得bash时**一定会读取**的配置文件
    + 本身**也是shellscript**，如果添加**权限x直接执行**也可以执行，但是**不会生效**，还是要通过`source /etc/profile`使得配置在**当前shell**生效
        + shellscript的执行时创建一个**子shell并在其中执行**，执行结束后**子shell退出**，自定义变量和一些配置也**随之失效**，但**执行结果**会返回**标准输出**，又因为父shell和子shell**共用一个屏幕设备或者屏幕文件**，会造成**似乎**是在父shell中执行一样
...
+ `[Ctrl]+S` 暂停**屏幕的输出**
+ `[Ctrl]+Q` 恢复**屏幕的输出**
+ **`[Ctrl]+U`** 在提示符下，**将整行**命令删除
+ `[Ctrl]+Z` 暂停目前的命令
# **Bash下的**通配符（wildcard）
+ **`*`** 代表**0到无穷多**个**任意**字符
+ **`?`** 代表**一定有一个**任意字符 
+ **`[]`** 代表一定有一个在**中括号内**的字符（**非任意**字符） `[abcdefg]`
+ **`[^]`** 代表**除了**中括号内的**任意一个**字符 `[^abcdefg]`
+ **`[-]`** 代表**编码顺序内**的其中一个字符 `[a-z]`
# **Bash下的**特殊符号
+ `#` 注释符号，其后数据**不执行**
+ `\` 转义符号，**特殊字符或者通配符**转为**一般字符（字面量）**
+ `|` 管道（pipe） 
+ `;` **连续执行**命令，和管道不同
+  `~` 用户的主文件夹
+  `$` 提取变量值
...
+  `!` 逻辑意义上的“非”（not）
+  `/` 目录符号
+  `>` `>>` 数据流重定向，**输出**导向，分别是**替换**和**累加**  `cat > testFile; [Ctrl]+D //按键结束输入` `cat > testFile << eof #输入eof结束输入`
+  `<` `<<` 数据流重定向，**输出**导向，分别是**替换**和**累加**
+  `''` 单引号，**变量失效**，视为**字面量**输出
+  `""` 双引号，变量有意义，可以**取值**输出
+  "``" 反引号，**作为命令执行**并将**执行结果**替换到**当前位置**，也可以使用`$()`
+  `()` 中间为**子shell**的起始与结束
+  `{}` 中间为**命令块**的组合
# shell的**变量**功能
+ **`echo`** 变量的**显示**（$和${}都有**取值**功能）  
1. 变量名称前加上$即可：`echo $var`
2. 变量名称用${}**包围**即可：`echo ${var}`
+ `set`和`unset` 变量的**设置**与**取消** `set`和`unset` 
1. 在bash当中，一个变量名称**尚未被设置**时，**默认**内容是“空”，echo该变量不显示任何信息
2. 变量的设置规则
    + 变量与变量内容以一个等号“=”来连接`myname=Eleseven`
    + 等号两边不能**直接接**空格符`myname= VBird`或`myname=VBird Tsail`
    + 变量名称**只能是**英文字母与数字，但是**开头字符**不能是数字`2myname=VBird`
    + 变量内容**若有空格符**可使用双引号""或单引号''将变量内容**结合起来**
        + **双引号**内的**特殊字符如$等**，可以保有**原本的特性**（依然**视为变量**）
            + 若`var="lang is $LANG"`，则`echo $var`可得`lang is en_US`
        + 单引号内的特殊字符则仅为**一般字符(纯文本)**（**不视为**变量）
            + 若`var='lang is $LANG'`，则`echo $var`可得`lang is $LANG`
    + 可用**转义字符“\”** 将**特殊符号**(如[Enter]、$、\、空格符、！等)变成**一般字符** 
    + 在一串命令中，还需要通过**其他的命令**提供的信息可以使用**反单引号**命令或$（命令)特别注意，那个是键盘上方的**数字键1左边**那个按键，而不是单引号。
        + 例如想要取得内核版本的设置：`version=$(uname -r)` 再`echo $version`可得`2.6.18-128.el5`
    + 若该变量为了**增加变量内容**时，则可用`$变量名称`或`$(变量)`**累加内容**
        + `PATH="$PATH":/home/bin`
...
1. `env` 查看环境变量
    ...
    + `$LANG`
        + 语系数据
        + 中文编码通常是**zh_CN.gb2312**或**zh_CN.UTF-8**
    + `$RANDOM` 随机量
        + **随机数生成器 /dev/random**
        + ```
            echo $RANDOM //随机生成0~32767间的数值
            declare -i number=$RANDOM*10/32678; echo $number //随机生成0~9间的数值
          ```
2. `set` 查看**所有**变量（包含**环境**变量和**自定义**变量）
    + `$PS1` **提示符**的设置
        + 每次按下[Enter]键去执行某个命令后，最后要再次出现提示符时，系统就会主动去读取这个变量的值
    + `$$` **本shell**的**PID**
    + `$?` 上个**命令**的**回传码**，可以用来判断命令执行**成功与否**
        + 命令执行成功，回传一个**0值**
        + 命令执行失败，回传一个**非0值**
    + `$OSTYPE $HOSTTYPE $MACHTYPE` 主机硬件与内核的等级
3. `export` 自定义变量**转成环境变量** 或者 **显示**所有**环境变量**
    + 当你登录Linux并取得一个bash之后，你的bash就是一个**独立的进程**，接下来**在这个bash**下面执行任何命令都是这个bash**衍生出来**的，被执行的**命令**称为**子进程**
    + 环境变量与自定义变量的**差异**在于该变量是否会被子进程**继续引用**，子进程**会继承**父进程的**环境变量**，**不会继承**父进程的自**定义变量**        
4. `locale` 影响显示结果的**语系变量**
    + `locale -a` Linux支持的语系
    + `locale` 显示相关变量
    + `/etc/sysconfig/i18n` 系统**默认语系**定义文件
+ 变量的**有效范围**（**作用域**）
1. 环境变量 = **全局**变量
2. 自定义变量 = **局部**变量
3. 子进程只继承父进程的环境变量，不继承父进程的自定义变量
+ `read` 变量**从键盘**读取值
1. `-p`：提示语
2. `-t`：等待键盘输入的秒数
3. ```
...
2. `-a`：声明为**数组（array）**
3. `-i`：声明为**整型（integer）**
4. `-r`：声明为**只读（readonly）** 不可修改和重设，不小心设置成只读，只能**注销再登录**才能恢复
5. ```
    declare -i sum=100+300+50; echo $sum // 输出 450
    sum=100+300+50;echo $sum //输出 "100+300+50"
...
3. **复原**ulimit设置：注销再登录
+ 变量**内容**的**删除**
1. `#` **从头开始**符合替换模式的**最短的**那一个
2. `##` **从头开始**符合替换模式的**最长的**那一个
3. `%` **从尾向前**符合替换模式的**最短的**那一个
4. `%%` **从尾向前**符合替换模式的**最长的**那一个
5. ```
    path=$PATH
    echo ${path#/*:} //仅删除第一个路径
    echo ${path##/*:} //仅剩下最后一个路径
...
1. ```
   ...
   ```
# `history` 显示历史命令
+ `~/.bash_history` 该文件记录**当前用户**的命令历史，会有**安全隐患**
+ 缺点是**无法记录**时间
+ ```
    history
    !number //执行第number条命令
   ...
   ```
# 数据流**重定向** 
+ **标准输出**（stdout,standard output）和标准**错误**输出（std**err**,standard **error** output）都是**默认输出到屏幕上**
+ 不管正确或错误的数据**都输出**到屏幕，屏幕是**混乱的**，数据重定向机制可以将两者分别传送到**其他的文件或设备**中去，**屏幕也是一种文件或设备**
+ 特殊字符及代码
1. 标准**输出**（stdout）：代码为1，使用`>`、`1>`或`>>`、`1>>`
    + **不存在**的文件会被**自动创建**
    + 若以`>`或`1>`输入一个**已存在**的文件，那个文件会被覆盖掉**从头开始**写入
    + 若以`>>`或`1>>`输入一个**已存在**的文件，会从那个文件**末尾继续**写入
2. 标准**错误**输出（std**err**）：代码为2，使用`2>`或`2>>`
3. **`/dev/null`** 垃圾桶**黑洞**设备 可以**吃掉**任何导向这个设备的信息
4. ```
    find /home -name .bashrc > list_right 2> list_error // 分别存到不同文件中去
    find /home -name .bashrc > list 2>&1 //同时存入一个文件
    find /home -name .bashrc &> list //同时存入一个文件
    find /home -name .bashrc > list 2> list //虽然前后都是list，但这是错误写法

    cat > testFile; 键盘输入内容;[Ctrl]+D //按键结束输入 
    cat > catfile < ~/.bashrc //catfile内容会和~/.bashrc一致
...
+ **`cmd;cmd`** **不考虑**命令相关性的**连续命令执行**
+ **`cmd1&&cmd2`** 考虑命令彼此的**相关性** （**一荣俱荣，一损俱损**）
1. 若cmd1执行完毕**且正确执行（$? = 0）**，则开始执行cmd2
2. 若cmd1执行完毕**且为错误（$? != 0）**，则**不执行**cmd2
+ **`cmd1||cmd2`** 考虑命令彼此的**相关性** （**择一而终**）
1. 若cmd1执行完毕**且正确执行（$? = 0）**，则**不执行**cmd2
2. 若cmd1执行完毕**且为错误（$? != 0）**，则**开始执行**cmd2
```
    ls /tmp/Eleseven && echo exist || echo "not found"
    ls /tmp/Eleseven || echo exist && echo "not found" //结果会同时显示 exist和not found
```
# 管道命令
+ 区别于**连续执行**命令
+ **注意事项**（**底层**理解）
...
2.管道后面接的**第一个数据**必定是“命令”，**而且**这个命令**必须要**能够接收**stdin的数据**，这样的命令才是**管道命令**
    ...
    + **非管道**命令：`ls` `cp` `mv`，它们**不会接收**来自stdin的数据
+ 管道命令实例
1. **`cut` `grep`** **选取**命令
    + **`cut`** 以行为**处理单位**，将**每行**数据根据分隔符分解成多个字段
        + `-d` 接**分隔符**
        + `-f` 取出**第几段**的意思，从1开始
        + `-c` 以**字符为单位**，取出固定字符区间
        + ```
            echo $PATH | cut -d ":" -f 5
            echo $PATH | cut -d ":" -f 3,5 //字段3和5
            export | cut -c 12- //取得第12个字符以后（**含第12个**）的所有字符
    + **`grep`** 解析**一行文字**，取得**关键字**，如果该行有存在关键字，就会**整行列出** （此处为简单了解，**正则表达式**部分会有更详细的说明）
        + `-c` 计算找到关键字的**次数**
        + `-i` `忽略大小写的不同`
...
2. **`sort` `uniq` `wc`** **排序**命令
    + **`sort`**
        ...
        + `-b` 忽略最前面的**空格**
        + `-M` 以月份名字排序 如 JAN DEC
        + `-n` 使用**纯数字**排序，默认是文字类型排序
        + `-r` 反向排序
        + `-u` 相同数据只取一行 uniq
        + `-t` 分隔符，不指明则默认是**以[Tab]键**分割
        + `-k` 分割后，选取某个字段 `-t`和`-k`常一起使用
        + ```
...
8.  退出：`q`
+ 正确关机
1. **`sync`**数据同步写入磁盘，将内存尚未被更新的数据写入硬盘，系统关机或重启之前最好多执行几次
2. 重启、关机命令
